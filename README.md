...
长时外部 Agent / Reviewer 若使用 `run_external_agent.mjs --terminal-receipt <path>`，执行结束后必须先原子写入 `external_agent_terminal` 回执；可同时用 `--result-path <path>` 指向已持久化的完整结果。随后 `scripts/terminal_continuation.py` 只按 Git common-dir 解析当前已登记的唯一 Controller，把该 receipt 作为 `SubagentStop` 控制事件写入既有 lifecycle state，并复用同一 `Wake Supervisor` 恢复该 Controller。恢复提示必须带 receipt 路径，Controller 先读持久结果再继续；成功的 `control_event_guard` 收据会清除已消费 receipt。此链不创建第二 Controller，也不新增 watchdog 或项目状态机。
## 台账怎么写才不拖慢开发
