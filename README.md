...
不提供 `UserPromptSubmit / Stop` 的宿主（包括无法暴露最终文本生命周期事件的 Web 宿主）无法由本机 Codex Hook 强制拦截最终回复。此时正式数值评分走 `controller_scoring_guard.py record-read --repo <project> --controller-session <id>` → `finalize-score --repo <project> --controller-session <id> --performance-score <raw> --governance-risk-status <GREEN|AMBER|RED> --risk-summary <evidence> --score <constrained> ...`；`record-read` 会把同一逻辑 Controller 的机器风险投影绑定到本次读模型收据，`finalize-score` 必须逐字匹配该风险状态并执行活动上限，不能用自报 `GREEN` 绕过 `49`。`control_event_guard.py` 为已登记 Controller 的成功或失败控制事件写入 Git common-dir 不可变机器证据收据；单回合回复只有携带匹配收据 ID 才能进入同 Controller、同模型的最佳 / 最差（最高 / 最低）记录，自报终态只能成为 `cycle_candidate`。正式回复必须分开报告“近期履职能力 / 治理风险状态与依据 / 风险约束分”；无合格单回合样本时最高 / 最低分与回合 ID 均写 `UNKNOWN`。正式七维原始分固定为 **0–100** 后按权重加权。重大治理事故的 `49` 分上限不按时间自然消失，只有机器收据证明纠正、规则 ACK 对齐和后续 L3 / L4 闭环后才解除；旧事故回合仍保留在历史极值中。低层 `score-guard` 只用于一次性兼容放行，不产生正式分。任一步未通过就禁止输出分数，不能把兼容路径描述成宿主级自动拦截。
