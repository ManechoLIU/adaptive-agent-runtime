...
| 外部模型路由 | 独立选择 Kimi K3 或 Grok 4.6，并为每条路线显式使用 OAuth 会员或 API Key；负责登录编排、无调用预检和授权执行，不依赖任务生命周期插件 |
...
```text
请使用 adaptive-delivery 配置外部 Agent：前端使用 Kimi K3，后端使用 Grok 4.6；分别让我选择 OAuth 会员或 API Key，先只做不调用模型的预检。真实调用前再次确认，不要静默切换认证或计费路线。
```
