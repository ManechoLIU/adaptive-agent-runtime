...
然后在 `~/.codex/hooks.json` 合并六个 command handler，命令都指向安装副本的 `scripts/lifecycle_hook.py`：`SessionStart`、`UserPromptSubmit`、`PreToolUse`（matcher `*`）、`PostToolUse`（matcher `*`）、`SubagentStop` 和 `Stop`。Codex 的非托管 Hook 还必须在 CLI 的 `/hooks` 中审核并信任精确定义。信任后先从 identity projection 取得 logical Controller、当前 Desktop execution target、target generation 与 ownership generation，再运行 `python3 ~/.agents/skills/adaptive-agent-runtime/scripts/lifecycle_hook.py --arm-desktop-canary <controller-id> --execution-target-session-id <desktop-target-id> --target-generation <generation> --ownership-generation <ownership-generation>`；随后由该 exact target 按顺序完成会话启动、普通工具前后事件、成功收据锁、同回合拒绝、Stop、新回合放行和 SubagentStop。Canary 绑定当前 Hook、lifecycle 与 target-guard 哈希、logical Controller、实际 execution target、两层 generation、随机 run ID 和严格事件顺序；不能把 Controller ID 当成 target，也不能跨 target 或跨 generation 拼接。安装器只有在该 8 步收据 24 小时内完整通过时才报告 `desktop_adapter=enabled`，其余情况保持 `degraded`。
