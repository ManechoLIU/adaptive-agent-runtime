...
Adaptive Agent Runtime 的 `Controller Health` 只从现有机器事实派生，`Wake Supervisor` 只负责把待处理控制事件送回**同一个** registered controller；public Skill ID 是 `adaptive-agent-runtime`，legacy machine state path 仍为 `adaptive-delivery`，no second controller，no mutable health ledger。正式常驻入口是宿主中立的 `scripts/controller_runtime_supervisor.py`：Desktop 的 canonical continuation 与 rule wake 不依赖 AI-Bridge；同一服务只有在可信 Web machine-event source 存在时才额外消费 Web Writer / Reviewer 事件。Desktop capability 只有在 plist 精确指向当前安装入口、且 90 秒内 heartbeat 带 `host_neutral_controller_runtime_v1` 合同标识时才报告 background continuation `ready`；仅配置未观察到新入口存活时报告 `configured_unverified`。历史 `web_agent_health_supervisor.py`、launchd label、plist 和 state path 继续作为兼容实现与迁移路径，不能据此把 Desktop continuation 误判为 Web 或 AI-Bridge 能力。main 与已明确绑定的总控 worktree 以 `Git common-dir` 共享同一所有权，但 controller worktree events require binding proof，普通 Writer / Reviewer worktree 不会变成总控。

原生桌面宿主把逻辑 Controller、Hook 来源和当前出站目标分开管理。`--bind-desktop-session` 只登记无执行权的历史别名；切换唯一当前入口必须使用 `python3 scripts/lifecycle_hook.py --replace-desktop-session <controller-id> <desktop-session-id> <repo> --expected-generation <n>`，退休则使用同样带 `--expected-generation <n>` 的 `--unbind-desktop-session`。这是 generation CAS：首次显式迁移 legacy 或 alias-without-target 用 `0`；已有 target 必须提供当前 generation，陈旧值或缺失值均 fail closed。replace CLI **只**机器校验 CAS、active lease 与 ownership，不能验证宿主的 interrupt / status / provenance；可信宿主或操作者必须在调用前完成该对账。本仓库不得把模型收据、审计引用或 CLI 成功冒充宿主停止证明。replace 会写入带 generation 的 `__controller_targets__`，让旧 canonical / alias 对**后续**受管 Hook 和出站恢复失活，但不撤销已启动动作；缺少上述宿主证据时不得对新 task resume / write。仅 `mcp__codex_app__send_message_to_thread`、`mcp__codex_app__navigate_to_codex_page`，以及显式 `threadId` 的 `mcp__codex_app__open_in_codex` 是受保护 Controller 定向动作：以完整 Hook payload 的 `tool_use_id` 持久持有 target-bound lease，直到匹配 `PostToolUse` 才释放；其他工具不在此覆盖宣称内。active lease 阻止 replace / unbind，缺 Hook、ID 或 PostToolUse 一律 fail closed，且 lease 不会自动过期。仅在可信宿主已核实 terminal / interrupted 后，管理员才可执行 `controller_target_guard.py reconcile --repo <repo> --host <host> --controller-id <controller-id> --tool-use-id <tool-use-id> --action <action> --target-session-id <id> --generation <n> --host-receipt-reference <reference> --reason <reason>`：它只释放精确匹配的 lease / target / generation，保留最近 64 条审计；引用只定位已核实事实，不是宿主证明。native wake 自动经过 `scripts/controller_target_guard.py`；调用这三类消息或导航前，先运行 `controller_target_guard.py check --repo <repo> --host desktop_codex --action message|navigate --target-session-id <id>`。标题、链接、alias 数组顺序和历史记忆都不是目标证据。

所有 `pending_control_event` 复用同一唤醒路径：`ACTIVE` 不重复启动，`DEFERRED` 保持 pending，宿主恢复走 same-controller continuation；当前不捆绑生产 peer host verifier，generic peer fallback 因此保持 degraded / `DEFERRED`。只有宿主提供并注册可信 host-attested verifier 后才可 `CONFIRMED` 并进入既有授权 fallback。`DEAD` 不会自动更换总控，而是 fail closed 并要求 explicit Resume/Replace handling。wake success does not clear pending；只有总控完成原控制事件并产生既有闭合证据后才清除 pending。
...
需要机器判定的最终 Reviewer 使用 `python3 scripts/reviewer_supervisor.py run --repo <repo> --base <branch>`。同一仓库同一时刻只允许一个活跃 Supervisor，通过 Git common-dir 下的进程锁 fail-closed 拒绝并发启动；Supervisor 只接受 clean worktree，把 base ref 和候选 HEAD 都冻结为精确 revision，使用只读结构化 `codex exec` 审查固定 `base_sha..head_sha`，并通过 Codex `--output-schema` 约束最终 JSON verdict；native `codex exec review` 仍可人工使用，但不再承担机器 verdict，因为当前 CLI 的 native review 最终格式可能忽略 schema。结束后 Supervisor 再核对 HEAD/status；PID 存活或 exit 0 单独都不是 Reviewer 已运行/已通过的证据。每次尝试有有界超时（默认 600 秒），启动时固定记录 Reviewer 进程组 ID；先对该固定进程组发 SIGTERM，即使 leader 已退出也继续确认后代是否存活，宽限期后仍有成员则 SIGKILL，只有 leader 已回收且整个进程组确认消失后才允许重试；无法确认清理时 fail-closed 且禁止重试。启动失败、stdin 提前断开、stdin 写入阻塞、wait/reap 异常、进程组信号失败等基础设施异常都必须受同一 attempt deadline 约束并落成持久 `REVIEW_INFRA_FAILED`，不能让 run 留在 `STARTING`；任何已启动子进程都先执行可验证的进程组清理，无法确认安全回收时禁止重试。最终 verdict 除了受 `--output-schema` 约束，本地 validator 还独立要求且只允许 `reviewed_head / verdict / critical / important / minor` 五个键。基础设施失败最多按同一不可变 review 合同自动重试一次，真实 findings 不为寻找不同 verdict 自动重试。


长时外部 Agent / Reviewer 若使用 `run_external_agent.mjs --terminal-receipt <path>`，执行结束后必须先原子写入 `external_agent_terminal` 回执；可同时用 `--result-path <path>` 指向已持久化的完整结果。随后 `scripts/terminal_continuation.py` 只按 receipt 自带的仓库身份与 Git common-dir 解析当前已登记的唯一 Controller，先同步把该 receipt 作为 `SubagentStop` 控制事件写入既有 lifecycle state，再由独立 wake 子进程复用同一 `Wake Supervisor` 恢复该 Controller；`DEFERRED` / 已有活跃执行不会反向改写已完成 Agent 的退出结果。未消费 receipt 在 `SessionStart` 仍保持 pending，恢复提示必须带 receipt 路径，Controller 先读持久结果再继续；只有真正闭合该 pending control event 的成功 `control_event_guard` 收据才清除已消费 receipt。此链不创建第二 Controller，也不新增 watchdog 或项目状态机。
## 台账怎么写才不拖慢开发
...
然后在 `~/.codex/hooks.json` 合并六个 command handler，命令都指向安装副本的 `scripts/lifecycle_hook.py`：`SessionStart`、`UserPromptSubmit`、`PreToolUse`（matcher `*`）、`PostToolUse`（matcher `*`）、`SubagentStop` 和 `Stop`。Codex 的非托管 Hook 还必须在 CLI 的 `/hooks` 中审核并信任精确定义。信任后运行 `python3 ~/.agents/skills/adaptive-agent-runtime/scripts/lifecycle_hook.py --arm-desktop-canary <controller-id> --repo /path/to/canonical/main`；arm 命令会在 registry 锁内自行解析当前 Desktop execution target、target generation 和 ownership generation，不接受调用者伪造这些字段。随后由该 exact target 按顺序完成会话启动、普通工具前后事件、成功收据锁、同回合拒绝、Stop、新回合放行和 SubagentStop。Canary 绑定当前 Hook、lifecycle 与 target-guard 哈希、logical Controller、实际 execution target、两层 generation、随机 run ID 和严格事件顺序；arm、每个事件及最终能力检测都会在当前 registry 中再次确认该项目只有这一个 logical Controller，并核对同一 registry 路径、target、ownership 与 generation，不能在 Controller 冲突期间积累观察，也不能跨 registry、跨 target、跨 ownership 或跨 generation 拼接。安装器只有在该 8 步收据 24 小时内完整通过时才报告 `desktop_adapter=enabled`，其余情况保持 `degraded`。
...
不提供 `UserPromptSubmit / Stop` 的宿主（包括无法暴露最终文本生命周期事件的 Web 宿主）无法由本机 Codex Hook 强制拦截最终回复。此时正式数值评分走 `controller_scoring_guard.py record-read --repo <project> --controller-session <id>` → `finalize-score --repo <project> --controller-session <id> --performance-score <raw> --governance-risk-status <GREEN|AMBER|RED> --risk-summary <evidence> --score <constrained> ...`；`record-read` 会把同一逻辑 Controller 的机器风险投影绑定到本次读模型收据，`finalize-score` 必须逐字匹配该风险状态并执行活动上限，不能用自报 `GREEN` 绕过 `49`。`control_event_guard.py` 为已登记 Controller 的成功或失败控制事件写入 Git common-dir 不可变机器证据收据；单回合回复只有携带匹配收据 ID 才能进入同 Controller、同模型的最佳 / 最差（最高 / 最低）记录，自报终态只能成为 `cycle_candidate`，且活动 `49` 上限同样约束单回合分。正式七维原始分固定为 **0–100** 后按权重加权；回复必须分开报告“近期履职能力 / 治理风险状态与依据 / 风险约束分”，无合格单回合样本时极值与回合 ID 均写 `UNKNOWN`。普通被门禁拦下的草稿不会自动制造 `49` 债；重大治理事故的上限不按时间自然消失，只有机器收据证明纠正 revision 已进 current main、作者 / 纠正 / Reviewer 身份绑定到当前台账中的成功终态任务、非作者审查与验收通过、已配置的跟踪远端可解析且与 main 一致、规则 ACK 对齐，并且后续 L3 / L4 明确依赖该纠正收据时才解除。同一事故有多次纠正时采用任一条完整精确证据链，不被更早的不完整尝试永久卡住。旧事故回合仍保留在历史极值中。低层 `score-guard` 只用于一次性兼容放行，不产生正式分。任一步未通过就禁止输出分数，不能把兼容路径描述成宿主级自动拦截。

ChatGPT Web + AI-Bridge 需要额外的 Web bridge，因为 Web 工具调用不会天然触发 Codex Hook。**repo 归属只能证明逻辑 Controller，不能证明当前 Web conversation 或桌面执行目标。** Legacy registry key 是稳定 `controller_id` / state lineage；事件中的 `controller_session_id` 保留该逻辑身份，`web_session_id` 单独表示经外部来源确认的 Web conversation/session，桌面 `execution_target_session_id` 由 `__controller_targets__` 当前 generation 决定，另以 `event_source=web` 和 `execution_host` 区分事件来源与实际执行宿主。Registry 的 `__controller_sessions__` 保存来源绑定，并通过 `bind-web-session --repo <repo> --controller-id <registered-controller> --web-session-id <trusted-web-session>` 在文件锁下原子写入；该命令**不会发现或猜测** Web session ID，调用方必须先从可信宿主来源取得它。同一 Web session 不能同时绑定两个 Controller。`post-shell`、`session-start`、`translate-receipt` 与 `arm-computer` 对 registered repo 都必须拿到匹配的 `--web-session-id`；常驻 `audit-once` 若启动参数没有该字段，只允许复用同一套仍有效的 `manual_user_authorized / resume_only` Web lease 解析出已绑定 session，否则 fail closed（78），禁止按 repo 自动冒认。当前 AI-Bridge shell/audit 若不暴露可信 Web conversation/session identity，正式可信路径仍必须 fail closed，不能声称宿主已经证明 Web Controller 身份。作为显式人工授权的临时降级，可先将 Web session 绑定到现有 Controller，再运行 `authorize-manual-web-session --repo <repo> --controller-id <registered-controller> --web-session-id <bound-web-session>` 创建默认 30 天、`resume_only` 的 repo/Controller scoped lease；zsh 自动桥优先使用宿主显式提供的 `ADAPTIVE_DELIVERY_WEB_SESSION_ID`，缺失时才解析该人工租约并把匹配的 Web session 注入 `post-shell`。人工租约**不是宿主级 caller identity 证明**：租约有效期间，任何能通过 AI-Bridge 在同一 registered repo 启动 shell 的 Web conversation 都无法被本机进一步区分，因此只能作为用户明确授权、单一 Web 总控使用场景的过渡方案；AI-Bridge 一旦提供 host-attested origin identity，应停止使用该降级路径。没有显式环境身份或有效人工租约时仍不桥接并保留原 shell 退出状态。未登记项目的 `post-shell` 仍静默跳过；同一 repo 若登记多个 controller 则拒绝映射；没有有效 Web identity/租约的 `computer` 事件不得猜测归属。Web 平台没有可验证的“最终文字回复已结束”本地事件，因此 bridge 不伪造这个信号：在真实 allowed 控制收据后做短 debounce，并可自动调用 `native-stop`；bridge 必须先经 target guard 恢复当前桌面 target，不能把逻辑 Controller ID 当作 task/thread 目标。更新的收据或 target generation 会使旧延迟 Stop 失效，不会 fork 第二 Controller。

`computer` 收据本身没有 repo / Web session ID，因此只允许**短租约归属**：只有已经取得可信 Web identity 的调用方才可在 canonical repo 内运行 `python3 scripts/web_lifecycle_bridge.py arm-computer --cwd <repo> --web-session-id <trusted-web-session>`；lease 同时绑定逻辑 Controller 与 `web_session_id`，默认只授权接下来 90 秒内 1 次 GUI 收据，可显式设置 `--ttl-seconds 5..300` 与 `--uses 1..8`；租约过期、耗尽、Controller 不匹配或 Web session 不匹配都不得归属该收据。`audit-once` 必须有 `--session-id <registered-controller>`，并优先使用显式 `--web-session-id <trusted-web-session>`；常驻 watcher 未显式携带 Web session 时，只可使用仍有效的人工授权 lease 解析同一已绑定 session，并在消费 audit 收据前同时核对 registry 与 Web binding。需要硬 Stop 时使用 `python3 scripts/web_lifecycle_bridge.py native-stop --session-id <registered-controller> --repo <repo>`，其中 `--session-id` 是逻辑 Controller；命令经 Guard 解析当前 `execution_target_session_id`，并在共享 registry 锁内把最终解析与 `codex exec resume` 进程启动绑定，不会 fork 或创建第二 Controller。常驻 `audit-once` 可加 `--auto-native-stop --auto-stop-delay-seconds 5`：每个真实 `control-event: allowed` 收据会安排一次延迟 native Stop，同 session 的更新收据会覆盖旧计划，target generation 变化也会使旧 debounce 失效。Web audit cursor 只在收据被持久处理成功后推进；同一 cursor 的消费从读取、`pending` 标记、dispatch、`handled` 持久化到 cursor 推进全程持有跨进程 consumer lock，并使用唯一临时文件原子写状态。可执行收据在处理前先写入 bounded `pending/handled` 去重记录，处理失败或进程在结果不确定时退出会保留 `pending` 并 fail closed，禁止盲目重复 dispatch，必须先对账后再恢复。Goal 收口事件同时必须在临时控制 JSON 中声明 `goal_rollover`，否则 `control_event_guard.py` 直接拒绝收据。
...
> 兼容说明：强纠正收据带机器能力版本；旧版弱纠正收据最多把风险推进到 `AMBER`，不能直接清债，后续闭环也不得绑定旧弱收据。
...
Adaptive Agent Runtime 的 Core 不依赖 AI-Bridge。检测到 AI-Bridge 且宿主同时提供**可信 Web session identity** 时，Web Adapter 才可把本地工具事件桥接进同一生命周期，并用 `scripts/web_lifecycle_bridge.py session-start --repo <repo> --web-session-id <trusted-web-session>` 生成 `AGENTS → TASK_LEDGER/PROJECT_STATUS → MEMORY → WIKI_INDEX → Git/runtime` 的恢复载荷；repo-only 映射不构成 Web 会话证明。没有 AI-Bridge、没有可信 Web session ID，或其他本地桥不可用时，Web 进入降级模式：仍可按 Core 规则工作，但不能声称看得到绑定后的本地 lifecycle、自动归属 Web 工具事件或代表该 Web conversation 唤醒 Controller。
