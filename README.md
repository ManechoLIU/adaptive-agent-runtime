...
- 只有整个项目确实没有任何内部恢复动作、无可执行 `READY`、且所有当前路径都有明确外部阻塞时，才把系统 Goal 标为 blocked。
...
Writer 开始前必须回传可送达总控的 ACK，包括工作树、分支、`HEAD/status`、精确文件范围、首个复现或 RED、验证和停止条件。任务创建成功、消息已发送、工作树 clean 或 Agent 显示 running，都不等于开发已经开始。
