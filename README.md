...
ChatGPT Web + AI-Bridge 需要额外的 Web bridge，因为 Web 工具调用不会天然触发 Codex Hook。**repo 归属只能证明 registered Controller，不能证明当前 Web conversation 就是该 Controller 的 Web 会话。** 现有 legacy registry key 仍是 Controller 的执行 thread/session 标识；事件中 `controller_id` 表示解析出的 Controller，`controller_session_id` 保留该 registered execution thread/session，`web_session_id` 单独表示经外部来源确认的 Web conversation/session，另以 `event_source=web` 和 `execution_host` 区分事件来源与实际执行宿主。Registry 的 `__controller_sessions__` 仅保存 Web 绑定映射，并通过 `bind-web-session --repo <repo> --controller-id <registered-controller> --web-session-id <trusted-web-session>` 在文件锁下原子写入；该命令**不会发现或猜测** Web session ID，调用方必须先从可信宿主来源取得它。同一 Web session 不能同时绑定两个 Controller。`post-shell`、`session-start`、`translate-receipt` 与 `arm-computer` 对 registered repo 都必须拿到匹配的 `--web-session-id`；常驻 `audit-once` 若启动参数没有该字段，只允许复用同一套仍有效的 `manual_user_authorized / resume_only` Web lease 解析出已绑定 session，否则 fail closed（78），禁止按 repo 自动冒认。当前 AI-Bridge shell/audit 若不暴露可信 Web conversation/session identity，正式可信路径仍必须 fail closed，不能声称宿主已经证明 Web Controller 身份。作为显式人工授权的临时降级，可先将 Web session 绑定到现有 Controller，再运行 `authorize-manual-web-session --repo <repo> --controller-id <registered-controller> --web-session-id <bound-web-session>` 创建默认 30 天、`resume_only` 的 repo/Controller scoped lease；zsh 自动桥优先使用宿主显式提供的 `ADAPTIVE_DELIVERY_WEB_SESSION_ID`，缺失时才解析该人工租约并把匹配的 Web session 注入 `post-shell`。人工租约**不是宿主级 caller identity 证明**：租约有效期间，任何能通过 AI-Bridge 在同一 registered repo 启动 shell 的 Web conversation 都无法被本机进一步区分，因此只能作为用户明确授权、单一 Web 总控使用场景的过渡方案；AI-Bridge 一旦提供 host-attested origin identity，应停止使用该降级路径。没有显式环境身份或有效人工租约时仍不桥接并保留原 shell 退出状态。未登记项目的 `post-shell` 仍静默跳过；同一 repo 若登记多个 controller 则拒绝映射；没有有效 Web identity/租约的 `computer` 事件不得猜测归属。Web 平台没有可验证的“最终文字回复已结束”本地事件，因此 bridge 不伪造这个信号：在真实 allowed 控制收据后做短 debounce，并可自动调用 `native-stop`，复用原 controller execution thread 走 Codex 原生生命周期；更新的收据会使旧延迟 Stop 失效，不会 fork 第二 controller。

`computer` 收据本身没有 repo / Web session ID，因此只允许**短租约归属**：只有已经取得可信 Web identity 的调用方才可在 canonical repo 内运行 `python3 scripts/web_lifecycle_bridge.py arm-computer --cwd <repo> --web-session-id <trusted-web-session>`；lease 同时绑定 registered controller execution session 与 `web_session_id`，默认只授权接下来 90 秒内 1 次 GUI 收据，可显式设置 `--ttl-seconds 5..300` 与 `--uses 1..8`；租约过期、耗尽、Controller 不匹配或 Web session 不匹配都不得归属该收据。`audit-once` 必须有 `--session-id <registered-controller>`，并优先使用显式 `--web-session-id <trusted-web-session>`；常驻 watcher 未显式携带 Web session 时，只可使用仍有效的人工授权 lease 解析同一已绑定 session，并在消费 audit 收据前同时核对 registry 与 Web binding。需要硬 Stop 时使用 `python3 scripts/web_lifecycle_bridge.py native-stop --session-id <registered-controller> --repo <repo>`；命令先核对 registry 精确匹配，再通过 `codex exec resume` 续接**同一 thread**，不会 fork 或创建第二 controller。常驻 `audit-once` 可加 `--auto-native-stop --auto-stop-delay-seconds 5`：每个真实 `control-event: allowed` 收据会安排一次延迟 native Stop，同 session 的更新收据会覆盖旧计划。Web audit cursor 只在收据被持久处理成功后推进；同一 cursor 的消费从读取、`pending` 标记、dispatch、`handled` 持久化到 cursor 推进全程持有跨进程 consumer lock，并使用唯一临时文件原子写状态。可执行收据在处理前先写入 bounded `pending/handled` 去重记录，处理失败或进程在结果不确定时退出会保留 `pending` 并 fail closed，禁止盲目重复 dispatch，必须先对账后再恢复。Goal 收口事件同时必须在临时控制 JSON 中声明 `goal_rollover`，否则 `control_event_guard.py` 直接拒绝收据。
