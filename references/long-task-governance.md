...
生命周期 state 同时记录最近真实控制事件的 `controller_host`：Web 桥收据标记 Web 宿主，原生桌面 Hook 标记桌面宿主；具体机器枚举只在模型路由参考中定义；同一 registered controller 跨端续接时随最新真实事件更新，禁止把 thread ID 固定等同于某个宿主。该字段只表示事件来源 / 就近偏好，不拥有 execution-target 权威：wake / continuation 必须先服从 `__controller_targets__` 的 canonical current target；当 lifecycle 偏好宿主已经没有可执行 current target，而另一宿主是唯一 explicit active target 时，必须复用同一 Controller 转向该 current target，禁止继续用历史 alias / lease 把 pending event 卡在旧宿主。只有偏好宿主本身仍有合法 current target（或仍符合严格 legacy 单入口兼容）时才保持该宿主。
