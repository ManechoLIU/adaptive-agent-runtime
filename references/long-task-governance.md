...
**Web Agent / Reviewer execution adapter**：Web Controller lifecycle bridge 与 Web 子 Agent execution 是两类不同身份，禁止把前者的 `web_session_id` 绑定逻辑直接当成后者的运行证明。Controller 创建并完成一次 Web Writer/Reviewer dispatch 后，`web_agent_execution.py start` 只把该 execution session、冻结的 Assignment 合同和当时可验证的 Git 基线写入 canonical runtime；新 Web Assignment 同时必须记录 `agent_id / provider / model / agent_type`，structured `collaboration.spawn_agent` 的机器事件必须与预登记的 `model / agent_type` 精确一致，否则绑定 fail closed，从而让后续 candidate、terminal 与事故追溯能够回答具体执行 Agent 和模型；它**不声称** browser Host generation 正在运行。start 同时写 `execution_transport=web`、`health_mode=progress_watchdog`；保留兼容字段 `exclusive_execution_key=task:<task_id>`，并写入 canonical `exclusive_execution_keys`。Writer 至少包含 `task:<task_id>` 与规范化 `worktree:<path>`，Reviewer 只保留任务互斥键，因此不同任务也不能并发写同一 worktree。正式安装由 Runtime 自身配置一个 `launchd KeepAlive` 的 **health-only scheduler**：它只写 readiness heartbeat，并周期调用既有 `web_lifecycle_bridge.reconcile_managed_web_assignments`；不实现第二套 wake/reentry，也不修改应用宿主 / 本地桥接宿主。reconciler 只从 canonical lease、结构化 `SubAgentActivity` 与可验证 progress 判断现场：它自行重算 Git HEAD / worktree status fingerprint，其他测试/证据/artifact/blocker 进展仍必须通过 canonical `assignment_progress` receipt；页面存在、PID、tab、DOM 文案、`Connection interrupted`、`missing` 等 UI/Host 信号都不得刷新 progress，也不得被提升为 terminal。结构化 `completed / failed / interrupted / cancelled / disconnected` 才能作为对应 execution session 的机器 terminal。对 `progress_watchdog` lease，PID/heartbeat lease 过期不作为 Web execution 的 terminal 或恢复依据；达到 progress deadline 后先进入既有 stale grace，超过 grace 才成为 `unhealthy:progress_stale_beyond_grace`。terminal 或 unhealthy 都统一经 `terminal_continuation → 既有 same-controller Web reentry supervisor` 交回**同一个** registered logical Controller；`web_reentry_unavailable` 这类瞬时 reentry 失败保留 pending 并重新 arm 同一 supervisor，禁止再造第二套 wake 状态机。被唤醒的 Controller 真正拿到新的 replacement execution session 后调用 `recover`；Runtime 从 canonical 合同自动派生严格 `attempt + 1` 与全新 lease，并继续共享原 recovery budget，调用方不能靠 UI 文案手工宣告旧 attempt 已结束。当前可自动恢复类别至少要求 v2 合同明确 `side_effect=false`；side-effect 状态/结果未知、legacy side-effect 合同未知，或仅存在一个未被 provider mutation boundary 机器证明的 idempotency key，全部 fail closed，不自动重试。旧 attempt 的迟到 receipt 继续由 attempt/session/lease fencing 拒绝。Reviewer start 仍必须冻结精确 immutable candidate revision，任何可信 terminal verdict 都必须通过与 `reviewer_supervisor.py` 相同的结构校验并绑定该 revision。若未来 Host 提供强 generation lifecycle API，可作为附加观测；当前 Host 没有可信 Host terminal API，因此安装 manifest 仍报 `web_agent_execution.status=host_limited` / `host_terminal=unavailable`；当 Runtime health service 已精确配置时同时记录 `health_supervisor=launchd_keepalive`、`continuation=existing_web_reentry_supervisor`、`recovery_mode=canonical_progress_health_supervisor`。断线/停滞恢复依赖 canonical Runtime + 结构化 collaboration 事件，而不是 Host 自报 terminal。 Web dispatch 还必须在 prepare/bind/start/recover 边界验证可信 machine-event source readiness；若当前 ChatGPT Web 宿主没有可机器证明的 child started/terminal 事件源，则 native Web child dispatch 必须 fail closed，不能用页面、tab、DOM 或浏览器动作冒充 Runtime 托管。Web execution adapter 只允许执行 canonical `provider=chatgpt_web`，不得把 Kimi/Grok 等外部 provider 的 route 标签带进 Web transport 后实际创建 ChatGPT 子会话；Kimi/Grok 必须继续由各自 canonical external-agent executor 执行。same-controller reentry 后、任何新 delegation 前，Controller 必须从完整 `TASK_LEDGER` 重算 DAG / READY / WIP 与 derived-runnable，并重新应用正式 route policy；不得因为最近一次执行器是 Web 就复用 Web 作为最短路径。
...
事实源、产品、视觉、技术、协作或验收规则变化时，变更者须向所有受影响的 live 任务发送精确版本、变更摘要、影响范围和新停止条件，并逐个取得 **loaded ACK**。Adaptive Agent Runtime 自身发布必须通过 `scripts/install_skill.py` 写入精确 revision、文件哈希、影响范围与停止条件的安装 manifest；升级只能沿当前正式安装 revision 的 Git 祖先链前进：候选仓库必须真实包含 installed revision，且 installed revision 必须是 candidate 的祖先，否则安装 fail closed。临时仓库或 detached hotfix 允许用于诊断和候选验证，但在正式安装前必须把该 hotfix Git lineage 吸收到 canonical Runtime 历史，禁止“安装副本比主仓库更新、后续开发从旧主线继续”的双事实源。完整 Web Runtime 候选还必须由安装器从 exact candidate revision 重新物化并执行固定的同一总控连续推进回归集：瞬时 Web reentry 失败必须重新 arm 既有 supervisor，Writer/Reviewer terminal 与 stale 必须回到同一 Controller，重复 terminal 不得重复 wake；缺少这些测试文件本身也视为发布门失败。registered controller lifecycle 自动比较 installed revision 与项目 Git common-dir 中的 `rule-handshake.json`，出现漂移时直接注入 `rule_update_pending:<revision>`，不依赖浏览器 / GUI 消息。规则更新的唤醒由机器按影响和当前执行事实分三档：命中正在运行 Assignment 的路由、运行时、交付或生命周期关键变化使用 `immediate`；影响后续 live Assignment 但允许当前原子事务安全收尾时使用 `after_event`，当前事务可闭合但不得开启新 Assignment，闭合后立即加载；不影响当前运行时行为的更新使用 `next_turn`，只在下一自然回合加载，不主动打断。三档都复用同一 handshake 和 lifecycle state，不新增通知台账，也不由模型临场猜测。总控读取新规则后用 `scripts/rule_handshake.py ack` 对 exact revision 生成机器回执，再把现有台账“规则版本”行同步到同一 revision；在 `impact=live_assignments` 时，ACK 或台账同步任一未完成，新的 Assignment-bound external Agent 都必须在 spawn 前 fail closed。文件已共享、消息已发送、安装已完成或台账单独更新都不能替代完整握手；无关工作继续。只有 installed revision、controller loaded receipt 与台账规则版本三者一致后才能宣称新规则对该项目生效。
