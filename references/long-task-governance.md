...
台账出现同一任务 ID 多处重复、顶部长期保留旧事故 / 旧 Reviewer / 旧漂移过程，或小状态变化产生大量纯台账提交时，立即做一次范围单一的减法整理：顶部只保留当前 Goal、活动任务、下一可见检查点、真实阻塞和规则版本；每个任务 ID 只出现一次；`ACTIVE / RECOVERING` 保留完整恢复字段，`READY / PENDING` 保留一行必要信息，`DONE` 压缩为结果、提交和证据链接。整理前后必须对账任务 ID、状态、依赖、授权门和证据集合，不能以瘦身为由删除仍有恢复价值的项目实例，也不新建历史台账。
...
项目同时存在多个 `READY`、必需 Reviewer 或规则更新 ACK 时，可在结束调度事件前用 `scripts/control_event_guard.py --ledger <唯一台账>` 校验一次临时 JSON。脚本以当前台账 SHA-256 和状态列为准核对全部 `READY`；必需 Reviewer 与规则更新的受影响 live 任务必须通过命令行参数显式声明后才能校验，脚本不会自行推断。输入不写入项目文件；门禁只替代确定性的完整性检查，不能代替总控对文件冲突、优先级、证据质量或真实完成的判断。
...
任务创建或消息发送不等于已经开工。Writer 的 delivered ACK 至少包含：绝对仓库根、分支与 `HEAD / status`、精确所有权、首个复现或 RED、停止条件；项目可按风险追加运行环境与禁止范围。Writer 在完整 delivered ACK 前不得写入。若握手迟延期间已经形成边界清楚的 RED WIP，总控先暂停该 Writer，核对实际文件、生产代码边界、越界和工具进展；无冲突时补齐 ACK 后从原 checkpoint 恢复，不因握手缺失自动丢弃有效 WIP，也不得在 ACK 闭合前标记 `ACTIVE`。第一次缺少 delivered ACK 时，总控必须主动定向 follow-up，要求尽快收缩到可见 checkpoint，不能因执行者没有主动汇报而放任不管。第二次判定中断前必须同时核对**消息送达、任务活动、工具事件、工作树状态**四类证据；这只是中断决策所需的即时核对，不新建表格、报告或治理文档，调度收据用一行简记结论即可。`clean` 只说明尚未写文件，不能证明 idle，running、依赖安装、测试或其他新工具证据都表示仍有进展。只要任一类有新增，就要求交付 RED checkpoint 而不误中断；只有四类证据均无新增时，才回收对应 Writer 并从最近现场重建，不连带中断其他任务。连续两次无新增后停止轮询，不把等待写成进展。
