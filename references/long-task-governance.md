...
浏览器托管的控制面通过本地工具桥操作项目时，如果宿主不产生原生生命周期事件，只允许做受限桥接：本地 shell 可在退出时把 canonical repo、真实命令和退出码映射为同一 controller 的 `PostToolUse`，前提是该 repo 恰好只有一个 registered controller；未登记项目静默跳过，多 controller 必须 fail closed。 Web Adapter 首次获得明确 repo 归属时可执行 `web_lifecycle_bridge.py session-start`，按 `AGENTS.md → 唯一台账 → MEMORY.md → WIKI_INDEX.md → Git/runtime` 生成 SessionStart 等价恢复载荷；缺失文档只表示该层暂无持久事实，不能编造。宿主在 repo 归属前没有可验证的 Web session/project identity 时必须明确降级，不能猜测绑定。控制门闭合还要回读工具桥的真实审计收据，只有 `control_event_guard.py` 输出包含 `control-event: allowed` 才能当成成功收据；不能仅凭 shell 退出码推断。没有 repo 归属的 GUI 事件不得猜测映射。宿主最终回复没有可验证的本地 `Stop` 回调时，桥接层不能伪称直接观测到“回复结束”；可在真实 allowed 控制收据后做短暂 debounce，再自动调用 `native-stop`，复用同一 registered controller session 走宿主原生生命周期。native resume 使用桥自身的确定性 runtime PATH，并在真正 resume 前预检 repo、唯一 controller、宿主 CLI 可执行文件、所需解释器 / runtime 与轻量版本检查；失败保持 `pending_control_event=true`，不能把 lifecycle 当作闭合。若 Codex thread-store 明确返回同一 thread `already has an active writer`，机器记录 `RESUME_DEFERRED_ACTIVE_WRITER`：这表示已有 writer 占有同一 controller thread，不是 peer-host 不可用，也不得因此创建第二总控或再次跨端；其他失败才按机器 failure class 判断是否属于既有 host fallback 允许集合。detached launcher 的 stdout/stderr 写入限量轮转日志，state 仅保留有界 head/tail diagnostics、精确 command、returncode 与时间戳，不再丢到 `/dev/null`。只有同一 controller resume 返回成功才写 `RESUME_CONFIRMED`。连续新收据会覆盖旧的延迟 Stop，只允许最新收据触发；任何时候都不创建第二个长期总控。由于 `goal_rollover` 已在控制收据层先强制，自动 native Stop 是第二层生命周期保险，而不是依赖 Web UI 时序猜测正确性。
