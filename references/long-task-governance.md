...
session_binding_state 单独描述当前执行入口：host / session_id / verification / reason / provenance / binding_mode / target_generation。VERIFIED 才允许 Controller-exclusive mutation；UNVERIFIED / STALE 只关闭当前入口的 Controller-exclusive side effects，不清除项目 ownership、Goal、pending lifecycle、Continuation Debt、runnable、Reviewer/candidate/recovery；CONFLICT 同时关闭恢复和 mutation。已有唯一 Controller 且 Session 未验证时，恢复结论固定为 SAME_CONTROLLER_SESSION_RECOVERY_REQUIRED，create_new_controller_allowed=false。只有 project_controller_state=ABSENT 才可进入 NEW_CONTROLLER_REQUIRED。
...
Project-context / SessionStart receipt 必须同时输出 project_controller_state 与 session_binding_state。例如唯一 Controller 已存在但 Web ID 不可得时，应显示 project Controller EXISTING / UNIQUE / ACTIVE，current execution UNVERIFIED / HOST_SESSION_ID_UNAVAILABLE / controller_actions_allowed=false / SAME_CONTROLLER_SESSION_RECOVERY_REQUIRED。恢复为 VERIFIED 后，restore payload 继续携带同一 Controller 的 pending_control_event / triggers / Continuation Debt / runnable_ids / candidate_revisions / next_action；只要 Goal 未完成且存在可安全继续的 mandatory work，就立即回到原 Controller control loop，而不是停在“身份恢复完成”。
