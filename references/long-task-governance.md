...
**Web Agent / Reviewer execution adapter**：Web Controller lifecycle bridge 与 Web 子 Agent execution 是两类不同身份，禁止把前者的 `web_session_id` 绑定逻辑直接当成后者的运行证明。Controller 创建 Web Writer/Reviewer conversation 后，只有宿主提供可验证的 generation lifecycle observation 时，`web_agent_execution.py` 才把 conversation/session 作为当前 Assignment attempt 的 `session_id` 写入 canonical runtime；start 同时写 `execution_transport=web` 与 `exclusive_execution_key=task:<task_id>`，与 `run_external_agent.mjs` 共用同一 exclusive gate，避免 Web/Desktop 双 Writer。新的 recovery attempt 可以换 execution session，但必须严格 attempt +1 且更换 lease；旧 attempt 的迟到 terminal/heartbeat 仍由 attempt/session fencing 拒绝。heartbeat 也必须来自经过 provenance / freshness / replay 校验的宿主 generation event，tab-presence 只能作为弱 UI 观察，既不得续 canonical lease，也不得刷新 progress；terminal `completed/interrupted/missing/failed` 同样必须来自 host-attested generation event，tab 消失、DOM 文案或页面存在都不足以证明 terminal。terminal 先写 `assignment_terminal`，再生成既有 `external_agent_terminal` receipt 并复用 `terminal_continuation.py → SubagentStop → Wake Supervisor` 唤醒同一个 registered Controller。Reviewer start 必须冻结 candidate revision，terminal verdict 必须通过与 `reviewer_supervisor.py` 相同的结构校验并精确匹配 candidate revision。若当前 Host 没有强 generation lifecycle API，安装 manifest 报 `web_agent_execution.status=host_limited`，该 transport fail closed，不得用刷新/无限 retry 伪装托管。
