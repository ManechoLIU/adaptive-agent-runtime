...
项目根目录不默认创建 `SKILL.md`。项目规则写入 `AGENTS.md`；只有形成独立、重复、专业化的项目能力时，才按宿主规范创建有明确名称的项目 Skill。
...
- 大面积改文件、长时间无证据、反复换方案、大规模返工或下一步不唯一时立即拆细。
- Agent 频繁等待、台账维护超过执行成本或机械项没有独立验收价值时合并。

状态使用 `PENDING / READY / ACTIVE / VERIFY / BLOCKED / DONE / SUPERSEDED`。同一时刻最多一个 `ACTIVE`，并保持唯一下一项。每个可执行项至少包含：目标、非目标、依赖、预计读写范围、验收标准、验证方式、证据位置、验证收据（检查 ID、候选快照、结果与失效条件）、暂停条件和下一步；高风险项再增加回滚、禁止动作和人类确认点。
...
普通问答、一次性审查、单文件低风险小改和无需恢复的短任务不创建 Goal。一个 Goal 只绑定一个台账项，或一个依赖紧密且能共同验收的小里程碑；不能把开放 backlog、多个高风险模块或整个项目塞入一个 Goal。

循环为：读取台账项 → 核对现场 → 标记 `ACTIVE` → 执行与验证 → 回写状态、证据、风险和唯一下一项 → 完成或阻塞 Goal。不得先结束 Goal、后补台账。

不同阶段使用相称闭环：开发项采用“实现与定向检查 → 真实业务 Case → 修复优化 → 审查全部 diff → 冻结候选 → 一次完整验证 → 范围单一的本地提交 → 台账回写”；需求、设计、调研和文档项用对应可观察产物与确认或链接证据，不为形式强行运行代码或操作界面。提交后内容快照未变则复用收据；推送后只核对远端落点，除非远端环境本身属于验收对象。

## 开工前候选与共享环境对账

开始新实现前，同时检查：

1. 当前仓库根、`HEAD`、分支、未提交 diff；
2. 全部 worktree、相关分支、未合并提交和候选产物；
3. 候选父链、实际 diff、测试与事实源版本；
4. 数据库、缓存、对象存储、端口、后台进程和输出目录是否被多个工作区共享。
...
Compact 不创建项目文件，也不作为完成证据。临时摘要至少保留：用户最新要求、当前 Goal / 台账项、目标与非目标、仓库根与 `HEAD` / 分支 / 未提交 diff、已完成及证据、仍有效的验证收据及其候选快照、未完成步骤、约束与禁止动作、失败路径、阻塞和唯一下一动作。
