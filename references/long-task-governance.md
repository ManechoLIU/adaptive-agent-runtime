...
Hook 只对登记的唯一总控和 canonical `main` 生效，Writer / Reviewer 工作树不受影响；它负责事件感知、工具轨迹与确定性门禁，不能代替总控判断文件冲突、优先级或 Assignment 内容。部分宿主工具路径可能不触发 Hook，因此项目台账、Git/runtime 对账和终态收据仍是第二道边界。非托管 Hook 必须完成运行平台对精确定义的审核与信任；随后显式 arm 一次绑定当前 hooks / lifecycle 哈希、单一 Controller 和随机 run ID 的 Canary，并在同一协议链按顺序观察 `SessionStart → PreToolUse allow → PostToolUse → receipt latch → same-turn deny → Stop → next-turn allow → SubagentStop`。安装器只有在该严格序列 24 小时内完成后才把桌面适配器标记为 `enabled`；跨会话、乱序或旧安装观察不能拼成通过收据，否则保持 `degraded`。
...
浏览器托管的控制面通过本地工具桥操作项目时，如果宿主不产生原生生命周期事件，只允许做受限桥接：canonical repo 只用于解析 registered Controller，**不能作为当前 Web conversation/session 的身份证明**。Legacy registry key 仍代表 Controller 的执行 thread/session；事件字段中 `controller_id` 表示解析出的 Controller，`controller_session_id` 表示 registered execution thread/session，`web_session_id` 才表示可信 Web conversation/session，`event_source` 与 `execution_host` 分别记录事件来源和实际执行宿主。Registry 的 `__controller_sessions__` 保存 Web binding，只有显式 `bind-web-session` 才可在锁内原子写入；它不负责发现 Web ID，调用方必须先从可信宿主来源取得。一个 Web session 只能归属一个 Controller。`post-shell`、`session-start`、`translate-receipt`、`arm-computer` 对 registered repo 都必须携带匹配的 Web session identity；常驻 `audit-once` 若启动参数未携带 Web session，只允许从仍有效的 `manual_user_authorized / resume_only` lease 解析同一已绑定 session，否则 fail closed，禁止按 repo 自动冒认；GUI lease 还必须把相同 `web_session_id` 写入租约并在消费时复核。当前工具桥如果根本不暴露可信 Web conversation/session ID，就明确降级：zsh 自动桥不生成 lifecycle 事件且不得覆盖原 shell 退出码，不得伪造绑定。Web Adapter 只有在上述身份成立时才可生成 `AGENTS.md → 唯一台账 → MEMORY.md → WIKI_INDEX.md → Git/runtime` 的 SessionStart 等价恢复载荷。控制门闭合还要回读工具桥的真实审计收据，只有 `control_event_guard.py` 输出包含 `control-event: allowed` 才能当成成功收据；不能仅凭 shell 退出码推断。没有 repo + verified Web identity 的 GUI 事件不得猜测映射。宿主最终回复没有可验证的本地 `Stop` 回调时，桥接层不能伪称直接观测到“回复结束”；可在真实 allowed 控制收据后做短暂 debounce，再自动调用 `native-stop`，复用同一 registered controller execution thread 走宿主原生生命周期。native resume 使用桥自身的确定性 runtime PATH，并在真正 resume 前预检 repo、唯一 controller、宿主 CLI 可执行文件、所需解释器 / runtime 与轻量版本检查；失败保持 `pending_control_event=true`，不能把 lifecycle 当作闭合。若宿主 thread-store 明确返回同一 thread `already has an active writer`，机器记录 `RESUME_DEFERRED_ACTIVE_WRITER`：这表示已有 writer 占有同一 controller thread，不是 peer-host 不可用，也不得因此创建第二总控或再次跨端；pending 必须留存，并由既有 auto-native-stop 以有界退避继续重试同一 controller，不能把“当前 writer 正忙”降级成等待用户再说继续；其他失败才按机器 failure class 判断是否属于既有 host fallback 允许集合。detached launcher 的 stdout/stderr 写入限量轮转日志，state 仅保留有界 head/tail diagnostics、精确 command、returncode 与时间戳，不再丢到 `/dev/null`。只有同一 controller resume 返回成功才写 `RESUME_CONFIRMED`。连续新收据会覆盖旧的延迟 Stop，只允许最新收据触发；任何时候都不创建第二个长期总控。由于 `goal_rollover` 已在控制收据层先强制，自动 native Stop 是第二层生命周期保险，而不是依赖 Web UI 时序猜测正确性。
