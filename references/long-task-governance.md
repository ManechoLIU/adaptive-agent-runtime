...
状态使用 `PENDING / READY / ACTIVE / VERIFY / BLOCKED / DONE / SUPERSEDED`。同一时刻最多一个 `ACTIVE`，并保持唯一下一项。每个可执行项至少包含：目标、非目标、依赖、预计读写范围、验收标准、验证方式、证据位置、暂停条件和下一步；高风险项再增加回滚、禁止动作和人类确认点。
...
不同阶段使用相称闭环：开发项采用“实现 → 定向与相关回归 → 真实业务 Case → 修复优化 → 复验 → diff 审查 → 范围单一的本地提交 → 台账回写”；需求、设计、调研和文档项用对应可观察产物与确认或链接证据，不为形式强行运行代码或操作界面。
...
Compact 不创建项目文件，也不作为完成证据。临时摘要至少保留：用户最新要求、当前 Goal / 台账项、目标与非目标、仓库根与 `HEAD` / 分支 / 未提交 diff、已完成及证据、未完成步骤、约束与禁止动作、失败路径、阻塞和唯一下一动作。
