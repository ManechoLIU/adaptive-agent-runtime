...
| `kimi-code` | `api` | `kimi-k3` | `KIMI_MODEL_API_KEY`, `MOONSHOT_API_KEY`, or OS credential store |
...
Kimi API selects `https://api.moonshot.cn/v1` when Kimi Code's local region is `mainland-cn`, otherwise `https://api.moonshot.ai/v1`. `KIMI_K3_BASE_URL` can explicitly override it. Keys from the `.com` and `.ai` platforms are not interchangeable.
