...
For Grok, stdin is only the Runtime-facing contract boundary. The Runtime must never forward a potentially large prompt through the Grok process argv. It writes the bounded prompt to a private temporary file (`0600`), invokes the official CLI with `--prompt-file`, and removes the temporary prompt after the attempt. `AD_GROK_MAX_PROMPT_BYTES` is a hard pre-spawn bound (default 128 KiB). A Reviewer contract above that bound fails before provider spawn with `failure_class=review_sharding_required`, `observed_bytes`, `max_bytes`, and `shard_target_bytes` evidence. The Controller must then split the same immutable candidate review into bounded, independently attributable Reviewer shards (default target 64 KiB each) and one final synthesis review; every shard and the synthesis must bind the same exact candidate revision. Writers or other roles are not silently split because arbitrary task sharding can change semantics; they fail with `prompt_too_large` and require an explicit new bounded Assignment.

A Grok attempt is also process-bounded, not merely Assignment-heartbeat-bounded. The headless `streaming-json` process runs in its own process group. The Runtime enforces a first-stdout deadline (`AD_GROK_FIRST_OUTPUT_TIMEOUT_MS`, default 90s), a no-stdout stall deadline (`AD_GROK_STALL_TIMEOUT_MS`, default 180s), and an absolute attempt deadline bounded by the Assignment progress deadline (`AD_EXTERNAL_ATTEMPT_TIMEOUT_MS` can lower that bound; unbound default 10m). Timeout cleanup sends SIGTERM to the whole process group, waits a bounded grace, escalates to SIGKILL, and verifies group disappearance. Failures are durable and classified as `first_output_timeout`, `generation_stalled`, `attempt_deadline_exceeded`, or `process_group_cleanup_failed`; cleanup uncertainty is not retry-safe. These watchdogs are transport safety, not proof that model output is semantically correct or project progress is complete.
