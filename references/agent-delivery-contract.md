...
Every downstream decision cites exact upstream evidence. A claim such as “reviewer approved” or “tests passed” without the relevant revision/receipt is insufficient for integration. Required machine review should run through `scripts/reviewer_supervisor.py`: it rejects a dirty worktree, captures the exact candidate HEAD before launch, revalidates HEAD/status after the attempt, injects the complete structured verdict contract with that exact HEAD, requires observable Codex execution plus a validated terminal verdict, and treats timeout/missing/malformed/stale output as `REVIEW_INFRA_FAILED`; a PID or exit code alone is never review evidence. Each attempt is time-bounded and terminates its Reviewer process group on timeout. Infrastructure failure may retry once under the identical immutable review contract, while actual findings are terminal review evidence and are not auto-retried. Once a required Reviewer returns `PASS`, the control event must either complete integration with machine-verified candidate ancestry in the exact current main plus current-main regression evidence or record a real `ordered_integration` queue checkpoint; `FAIL` must return the same candidate to an acknowledged rework Assignment. These are transition evidence, not new lifecycle states.
