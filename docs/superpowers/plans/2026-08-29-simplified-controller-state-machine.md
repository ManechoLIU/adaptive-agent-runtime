...
- [x] Write failing tests for all eight legacy-state mappings and runtime health overrides (`progress_stale`, unhealthy recovery, budget exhausted).
- [x] Run `python3 -m unittest tests.test_controller_state -v` and confirm RED because helper is missing.
- [x] Implement the pure mapping helper with no file I/O.
- [x] Expose `task_projection` from `lifecycle_hook.project_snapshot()` for ledger tasks, deriving runtime health only from current lease evidence.
- [x] Run controller-state and governance tests; confirm GREEN.
- [x] Commit `feat: add five-state controller projection`.
...
- [x] Add failing runtime tests proving repeated/no-delta `assignment_progress` refreshes heartbeat only, not progress deadline; each changed fingerprint refreshes progress.
- [x] Add failing tests proving attempts 1/2/3 preserve `recovery_count` 0/1/2, and an unhealthy/terminal attempt at count 2 derives `budget_exhausted` for another same-strategy recovery.
- [x] Implement fingerprint comparison and recovery-count persistence minimally.
- [x] Add lifecycle failing test that budget exhaustion surfaces `recovery_budget_exhausted:<task>` even with a current heartbeat/PID.
- [x] Implement lifecycle trigger using derived runtime decision; do not create a new ledger state.
- [x] Run runtime + governance suites; confirm GREEN.
- [x] Commit `feat: bound assignment recovery and evidence progress`.
...
- [x] Add failing Node tests: Assignment-bound execute without `--assignment-ack` fails before fake runner spawn; wrong assignment ID/repository root/HEAD fails before spawn; exact ACK allows spawn.
- [x] Extend lease guard with exact expected repository/head/assignment checks while keeping existing callers compatible.
- [x] Implement runner preflight: load JSON, invoke/replicate validated lease semantics, compare exact Git facts, then spawn only after success.
- [x] Keep unbound compatibility executions (no Assignment identity fields) unchanged.
- [x] Run governance + Node routing tests; confirm GREEN.
- [x] Commit `feat: gate external agent launch on delivered ack`.
...
- [x] Add failing tests for PASS without integrate/ordered queue, FAIL without rework, integrate without main revision, integrate without regression evidence.
- [x] Add positive tests for PASS+integrate+main/regression, PASS+ordered queue, FAIL+rework.
- [x] Implement cross-validation by candidate revision; require required review `candidate_revision` when verdict is present.
- [x] Preserve existing low-risk reviews without verdict semantics unless they declare a candidate revision/verdict.
- [x] Run governance suite; confirm GREEN.
- [x] Commit `feat: enforce review to integration closure chain`.
...
- [x] Update docs without duplicating the entire spec.
- [x] Run `python3 -m unittest discover -s tests -p 'test_*.py'`.
- [x] Run `node --test tests/external-agent-routing.test.mjs`.
