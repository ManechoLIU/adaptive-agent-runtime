...
10. 持续项目启用 [long-task governance](references/long-task-governance.md) 的**生命周期执行器**与 `Controller Health / Wake Supervisor`：每次控制事件和 Stop/Yield 都从唯一台账、runtime、candidate、规则握手与 Git 事实重算当前 snapshot 和 runnable；任一可执行工作、待审 / 待集成候选或恢复动作存在时禁止静默 idle。Web Bridge 与桌面 Hook 是本 Skill 自带的两个宿主适配器，共用同一 Controller、机器投影和四门；桌面成功控制收据在当前 `turn_id` 设置强制 yield，注册总控的收据必须绑定真实机器工具轨迹。只有项目级扫描证明全部开放包等待同一真实外部条件才允许 project blocked；宿主续接只复用同一 registered controller，失败保持 `pending_control_event` 并 fail closed，不创建第二总控。桌面 Hook 配置存在不等于生效，精确定义未获宿主信任或未通过当前安装的实机生效验证时，适配器保持 `degraded`。
11. Controller 身份三分：`controller_id` 是逻辑归属，`source_session_id` 是入站来源，`execution_target_session_id` 是唯一出站目标。任何 Web 新会话在自称 Controller、报告自身 Controller 评分或执行 Controller 专属动作前，必须先运行 `web_lifecycle_bridge.py controller-identity` 并取得当前可信 `web_session_id` 对应的 `VERIFIED` identity receipt；repo/tool access、历史任命、聊天记忆、registered controller 存在或人工 `resume_only` lease 都不能替代当前会话身份证明。没有证明即 `UNVERIFIED`，不得冒认。仅受保护的消息、导航和显式线程目标的打开任务以 `tool_use_id` 持久持有 lease，直到 `PostToolUse` 才释放；lease、完整 Hook 或工具 ID 缺失时 fail closed，replace / unbind 不得越过 active lease。replace 只切未来路由；旧任务先取得宿主 interrupt、status、provenance 对账证据，否则不得 resume / write。未捆绑生产 peer verifier，故 fallback 保持 degraded / deferred。歧义或宿主不可拦截时 fail closed。
