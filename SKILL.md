...
- 用户要求审计 / 评分项目总控履职、比较近期表现或检查“假繁荣”时：使用 [controller performance scoring](references/controller-performance-scoring.md)。正式七维原始分固定为 **0–100**。支持 `UserPromptSubmit + Stop` 的宿主启用 `controller_scoring_hook.py`；不提供 UserPromptSubmit / Stop 的宿主执行 `controller_scoring_guard.py record-read` 与 `finalize-score`，未通过则禁止输出分数。比较先用 `latest-score`；**单回合**诊断记录最佳 / 最差，不能覆盖正式评分。
