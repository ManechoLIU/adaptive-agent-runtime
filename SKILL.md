...
11. Controller 身份与执行授权分层：controller_id 是项目长期唯一 ownership；Session 是临时载体，Binding 是机器证明，Authorization 决定当前入口权限。project_controller_state 与 session_binding_state 必须独立。已有唯一 Controller 时，UNVERIFIED / STALE session 仅关闭当前入口的 Controller-exclusive mutation，不撤销 ownership、不要求重新任命、不允许第二 Controller；进入 SAME_CONTROLLER_SESSION_RECOVERY_REQUIRED。可信新 session 只能绑定 existing controller_id 并推进 target generation；无 Host verifier 时保持 degraded / fail closed，用户任命、repo 路径或模型自报不能替代机器身份。
