...
def controller_identity_receipt(
    *, repo: Path, registry_path: Path, web_session_id: str | None
) -> tuple[dict[str, Any], bool]:
    """Return a fail-closed Web Controller identity projection."""
    controller_id = registered_controller_for_repo(repo, registry_path)
    if controller_id is None:
        raise ValueError(f"no registered controller for {repo}")
    supplied = str(web_session_id or "").strip()
    verified = bool(
        supplied
    ...
    payload: dict[str, Any] = {
        ...
        "repo_access": True,
        "controller_id": controller_id,
        "controller_identity": "VERIFIED" if verified else "UNVERIFIED",
        "controller_actions_allowed": verified,
        "event_source": "web",
    }
    if supplied:
        payload["web_session_id"] = supplied
    if verified:
        payload["proof"] = "registered_web_session_binding"
    else:
        payload["reason_code"] = "WEB_SESSION_IDENTITY_REQUIRED"
...
def build_parser() -> argparse.ArgumentParser:
    ...
    controller_identity = subparsers.add_parser("controller-identity")
    controller_identity.add_argument("--repo", required=True)
    controller_identity.add_argument("--registry", default=str(DEFAULT_REGISTRY))
    controller_identity.add_argument("--web-session-id")
    controller_identity.add_argument("--receipt")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    if args.command_name == "controller-identity":
        repo = canonical_root(args.repo)
        registry_path = Path(args.registry).expanduser()
        try:
            payload, verified = controller_identity_receipt(
                repo=repo,
                registry_path=registry_path,
                web_session_id=args.web_session_id,
            )
            ...
            print(str(exc), file=sys.stderr)
            return 2
        if verified and args.receipt:
            receipt_path = Path(args.receipt).expanduser()
            receipt_path.parent.mkdir(parents=True, exist_ok=True)
            _write_json_atomic_file(receipt_path, payload)
        print(json.dumps(payload, ensure_ascii=False))
        return 0 if verified else 78
