...
def apply_receipt(state: dict[str, Any], receipt: dict[str, Any], now: datetime | None = None, policy: RuntimePolicy | None = None) -> dict[str, Any]:
    ...
    if event == "assignment_started":
        ...
        lease.update({
            ...
            "candidate_revision": str(receipt.get("candidate_revision") or "").strip() or None,
            "auth_mode": str(receipt.get("auth_mode") or "").strip() or None,
            "policy_class": str(receipt.get("policy_class") or "").strip() or None,
            "route_decision": str(receipt.get("route_decision") or "").strip() or None,
            "route_contract": json.loads(json.dumps(receipt.get("route_contract"))) if isinstance(receipt.get("route_contract"), dict) else None,
