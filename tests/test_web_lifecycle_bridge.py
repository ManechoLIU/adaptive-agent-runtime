...
class WebLifecycleBridgeTests(unittest.TestCase):
    ...
    def test_controller_identity_refuses_repository_only_attribution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            ...
            result = self.run_bridge(
                "controller-identity", "--repo", str(repo), "--registry", str(registry),
                "--receipt", str(receipt),
            )
            ...
            self.assertEqual(payload["controller_identity"], "UNVERIFIED")
            self.assertFalse(payload["controller_actions_allowed"])
            self.assertEqual(payload["reason_code"], "WEB_SESSION_IDENTITY_REQUIRED")
            self.assertFalse(receipt.exists())

    def test_controller_identity_writes_verified_receipt_only_for_bound_web_session(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            repo = root / "repo"
            repo.mkdir()
            ...
            result = self.run_bridge(
                "controller-identity", "--repo", str(repo), "--registry", str(registry),
                "--web-session-id", "web-current", "--receipt", str(receipt),
            )

            self.assertEqual(result.returncode, 0, result.stderr)
            payload = json.loads(result.stdout)
            self.assertEqual(payload["controller_identity"], "VERIFIED")
